...
- **2026-04-16:** Next feature planned: **Tax snapshot / annual financial export** — one-click per-property-per-year financial summary (TP-128-V format for Quebec). Requires `mortgages` table + new expense categories (school_tax, mortgage_interest, advertising, legal, utilities_landlord). Output: ZIP with summary PDF + CSV + all receipts. Financial data stays in DB, not sent to Claude API. Privacy: DPA (Data Processing Agreement) recommended for Julia.
